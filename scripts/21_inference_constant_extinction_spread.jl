...
fpaths = Glob.glob("data/simulations/constant_extinction_spread/*.tre")


n_iters = length(fpaths)

io = open("output/prog_constant_extinction_spread.txt", "w")

completed_jobs = [
                  split(Base.basename(x), ".")[1] for x in Glob.glob("output/simulations/constant_extinction_spread/jld2/*.jld2", scratch)
 ]
...
for fpath in fpaths
    ...
    try
        ...
        ## save data
        fpath = string(scratch, "output/simulations/constant_extinction_spread/newick/", name, ".tre")
        writenewick(fpath, data, rates)

        fpath = string(scratch, "output/simulations/constant_extinction_spread/rates/", name, ".csv")
        CSV.write(fpath, rates)

        fpath = string(scratch, "output/simulations/constant_extinction_spread/jld2/", name, ".jld2")
