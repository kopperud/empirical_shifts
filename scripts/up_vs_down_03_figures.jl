...
N_estimated = zeros(Float64, ntrees, 36, 36)
N_estimated[:,:,:] .= NaN
magnitudes_estimated = zeros(ntrees)
magnitudes_estimated[:] .= NaN
ntip = zeros(ntrees)
ntip[:] .= NaN
for (i, path) in enumerate(datapaths)
    tree_index = parse(Int64, split(basename(path), ".")[1])
    N_estimated[tree_index,:,:] .= load(path, "Nsum")
    ...
    mag = sum(N_estimated[tree_index,:,:] .* Δr) / sum(N_estimated[tree_index,:,:])
    magnitudes_estimated[tree_index] = mag
end


println("mean magnitude (true): \t $(mean(rm_na(magnitudes_true)))");
println("mean magnitude (estimated, unknown λ,μ,η): \t $(mean(rm_na(magnitudes_estimated)))")

println("median magnitude (true): \t $(median(rm_na(magnitudes_true)))");
println("median magnitude (estimated, unknown λ,μ,η ): \t $(median(rm_na(magnitudes_estimated)))")
...
hist!(ax2, rm_na(magnitudes_estimated), bins = 30, strokecolor = :black, strokewidth = 1)
linkaxes!(ax1, ax2)
...
df = DataFrame(
    ...
    "error" => error,
    "height" => tree_heights,
    "magnitude_estimated" => magnitudes_estimated,
