...
fpaths <- Sys.glob("output/simulations/grafts/downshift/newick/*.tre")[1:10]
...
ps_left[[1]] <- ps_left[[1]] + ggtitle("true speciation = [0.325, 0.13]")
ps_middle[[1]] <- ps_middle[[1]] + ggtitle("true extinction = 0.235")
ps_right[[1]] <- ps_right[[1]] + ggtitle("true netdiv = [0.09, -0.105]")
...
ggsave("figures/ten_trees_upshift_downshift.pdf", p, width = 450, height = 600*1.0, units = "mm", limitsize=FALSE)
